...
def dataloader(config, device):
    ...
    if config['MODEL_NAME'] == 'general':
        START_INPUT, END_INPUT, START_OUTPUT, END_OUTPUT  = 0, 4, 4, 6
    elif config['MODEL_NAME'] == 'specific':
        START_INPUT, END_INPUT, START_OUTPUT, END_OUTPUT  = 0, 2, 2, 4
    elif config['MODEL_NAME'] == 'pendulum':
        START_INPUT, END_INPUT, START_OUTPUT, END_OUTPUT  = 2, 4, 4, 6
    else:
        START_INPUT, END_INPUT, START_OUTPUT, END_OUTPUT  = 1, 5, 5, 7
        #START_INPUT, END_INPUT, START_OUTPUT, END_OUTPUT  = 3, 7, 7, 9
...
class FF_Baseline(nn.Module):
    ...
    def __init__(self, id, X, Y, layers, device, config, N_u):
        ...
        #     layers_list.append(nn.BatchNorm1d(layers[i+1]))
        #     if i < len(layers) - 2:
