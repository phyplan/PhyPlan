...
def extract_ep(filename):
    match = re.match(pattern, filename)
    if match:
        ep, time, cur_dis, cur_vel  ,friction,init_vel, i = match.groups()
        ep = int(ep)
        i = int(i)
        return int(ep), float(time), float(cur_dis), float(cur_vel)  ,float(friction),float(init_vel), int(i)
