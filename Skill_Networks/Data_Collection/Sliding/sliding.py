...
friction = 0.2 #or any other constant
...
for ep in range(start_ep,end_ep):
    ...
    while True:
        ...
        if config['PERCEPTION']:
            ...
            if cur_dis>= -3 and cur_dis <= 3 and box_states["pose"]["p"][0][2] <= 3 and box_states["pose"]["p"][0][2] >= -3 and pt > 3:
                gym.write_camera_image_to_file(sim, env, camera_handle, gymapi.IMAGE_COLOR, rgb_filename)
        else:
            ans = str(sim_num) + ' , ' + str(data_num) + ' , ' + str(friction) + ' , ' + str(init_vel) + ' , ' + str(t - start_time) + ' , ' + str(cur_vel) + '\n'
