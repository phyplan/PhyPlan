...
class Env(Environment):
    ...
    def setup(self, gym, sim, env, viewer, args):
        ...
        ball_id = pb.loadURDF("./assets/no_mass_ball.urdf", basePosition=[0, 0, 2], baseOrientation=pb.getQuaternionFromEuler([0, 0, 0]))
        ...
        scene = Scene([ball, pendulum, wedge, bridge, goal], {'sliding': torch.load('new_pinn_models/PINNSliding_p_0.pt').to(torch.device('cuda' if torch.cuda.is_available() else 'cpu')),
    ...
    def setup_tool(self, gym, sim, env, config, tool_type):
        ...
        for i in range(len(self.actives)):
            for j in range(len(self.actives[i])):
                if self.actives[i][j]:
                    ...
                    if self.tool_classes[i] == 'wedge':
                        pb.resetBasePositionAndOrientation(self.pinnSim.objects[2].pb_id, [20, 10, 0], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
                    elif self.tool_classes[i] == 'pendulum':
                        pb.resetBasePositionAndOrientation(self.pinnSim.objects[1].pb_id, [10, 10, -0.01], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
                        self.pinnSim.objects[1].update_joint_angle('pendulum_joint', self.PENDULUM_INIT_ANGLE)
                        self.pinnSim.objects[1].update_joint_angle('joint', 0.3)
                        ...
                        pb.resetBasePositionAndOrientation(self.pinnSim.objects[3].pb_id, [15, 10, -0.1], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
        ...
        self.bnds.append((0.5, 3.5))
        self.action_names.append("Ball Drop Height")
        ...
        if tool_class1 == 'wedge':
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[2].pb_id, [self.locs[0][0], self.locs[0][1], 0.0], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            self.pinnSim.objects[2].restitution = self.tool_instances[0][self.tool_type[1]]
        elif tool_class1 == 'pendulum':
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[1].pb_id, [self.locs[0][0], self.locs[0][1], -0.01], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            self.pinnSim.objects[1].restitution = self.tool_instances[1][self.tool_type[1]][1]
            self.pinnSim.objects[1].mass = self.pendulum_masses[self.tool_instances[1][self.tool_type[1]][0]]
        elif tool_class1 == 'bridge':
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[3].pb_id, [self.locs[0][0], self.locs[0][1], -0.1], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            self.pinnSim.objects[3].friction = self.tool_instances[2][self.tool_type[1]]
    ...
    def generate_random_state(self, gym, sim, env, viewer, args, config):
        ...
        goal_center = [0, 0]
        while abs(goal_center[0]) <= 0.15 or abs(goal_center[1]) <= 0.15:
            goal_center[0] = np.random.uniform(-0.45, 0.45)
            goal_center[1] = np.random.uniform(-0.45, 0)
        pb.resetBasePositionAndOrientation(self.pinnSim.objects[-1].pb_id, [goal_center[0], goal_center[1], 0], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
        body_states = gym.get_actor_rigid_body_states(env, goal_handle, gymapi.STATE_ALL)
        for i in range(len(body_states['pose']['p'])):
            body_states['pose']['p'][i][0] += goal_center[0]
    ...
    def execute_action(self, gym, sim, env, viewer, args, config, action, return_ball_pos=False):
        ...
        ball_states['pose']['p'][0] = (0.0, 0.0, action[curr_action])
    ...
    def execute_action_pinn(self, config, goal_pos, action, time_resolution=0.01, render=False):
        ...
        curr_action = 0
        if self.tool_type[0] == 0:
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[2].pb_id, [0.0, 0.0, 0.0], pb.getQuaternionFromEuler([0.0, 0.0, action[curr_action]]), physicsClientId=self.pinnSim.pb_cid)
            pb.changeVisualShape(self.pinnSim.objects[2].pb_id, -1, rgbaColor=self.colours[0][self.tool_type[1]])
            for li in range(pb.getNumJoints(self.pinnSim.objects[2].pb_id)):
                pb.changeVisualShape(self.pinnSim.objects[2].pb_id, li, rgbaColor=self.colours[0][self.tool_type[1]])
            curr_action += 1
        elif self.tool_type[0] == 1:
            self.pinnSim.objects[1].update_joint_angle('joint', action[curr_action])
            self.pinnSim.objects[1].update_joint_angle('pendulum_joint', action[curr_action + 1])
            self.pinnSim.objects[1].joint_omegas['pendulum_joint'] = 0.0
            pb.changeVisualShape(self.pinnSim.objects[1].pb_id, -1, rgbaColor=self.colours[1][self.tool_type[1]])
            for li in range(pb.getNumJoints(self.pinnSim.objects[1].pb_id)):
                pb.changeVisualShape(self.pinnSim.objects[1].pb_id, li, rgbaColor=self.colours[1][self.tool_type[1]])
            curr_action += 2
        elif self.tool_type[0] == 2:
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[3].pb_id, [0.0, 0.0, -0.1], pb.getQuaternionFromEuler([0.0, 0.0, action[curr_action]]), physicsClientId=self.pinnSim.pb_cid)
            pb.changeVisualShape(self.pinnSim.objects[3].pb_id, -1, rgbaColor=self.colours[2][self.tool_type[1]])
            for li in range(pb.getNumJoints(self.pinnSim.objects[3].pb_id)):
                pb.changeVisualShape(self.pinnSim.objects[3].pb_id, li, rgbaColor=self.colours[2][self.tool_type[1]])
            curr_action += 1
        pb.resetBasePositionAndOrientation(self.pinnSim.objects[0].pb_id, [0.0, 0.0, action[curr_action]], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
        curr_action += 1
