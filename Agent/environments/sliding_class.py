...
class Env(Environment):
    ...
    def setup(self, gym, sim, env, viewer, args):
        ...
        # box_size = 0.1
        # asset_options = gymapi.AssetOptions()
        # asset_options.density = 100000
        # box_asset = gym.create_box(sim, box_size, box_size, box_size, asset_options)
        # pose_box = gymapi.Transform()
        # pose_box.p = gymapi.Vec3(0, 0, 0.4 + 0.5 * box_size)
        ...
        ball_id = pb.loadURDF("./assets/puck.urdf", basePosition=[0, 0, 0.315], baseOrientation=pb.getQuaternionFromEuler([0, 0, 0]))
        ...
        scene.setup({"puck": 1.0, "pendulum": 1.0, "sliding table": 1.0}, \
                    {"puck": self.FRICTION_COEFF, "pendulum": 0.0, "sliding table": self.FRICTION_COEFF}, \
                    {"puck": True, "pendulum": True, "sliding table": False},\
                    {"puck": 0.0025, "pendulum": 0.5, "sliding table": 20})
    ...
    def execute_action(self, gym, sim, env, viewer, args, config, action, return_ball_pos=False):
        ...
        # props = gym.get_actor_dof_properties(env, pendulum_handle)
        # props["driveMode"] = [gymapi.DOF_MODE_POS, gymapi.DOF_MODE_NONE]
        # props["stiffness"] = (1e10, 0.0)
        # props["damping"] = (40, 0.0)
        # gym.set_actor_dof_properties(env, pendulum_handle, props)
    ...
    def execute_action_pinn_old(self, config, goal_pos, action, time_resolution=0.01):
        ...
        # print('puck_vel', puck_vel.item())
        # print(vx, vy, vz)

        t_query = time_resolution
        while True:
            inp = torch.tensor([abs(puck_vel), t_query]).to(device).unsqueeze(0)
            [vel, distance] = model_sliding(inp)[0]
            # print("VEL:", vel.item())
            # print("DIST:", distance.item())
            # print("############################", distance.item())
            if vel < 0.01:
                break
            t_query += time_resolution
        ...
        x_new, y_new = -distance * vx / puck_vel, -distance * vy / puck_vel
    ...
    def execute_action_pinn(self, config, goal_pos, action, time_resolution=0.01, render=False):
        ...
        pb.resetBasePositionAndOrientation(self.pinnSim.objects[1].pb_id, [-0.035*np.sin(phi), 0.035*np.cos(phi), 0.3], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
    ...
    def get_final_state_via_PINN(self, config, action, time_resolution=0.01):
        ...
        x_new, y_new = -dist* vx / puck_vel, -dist * vy / puck_vel
        z_new = torch.tensor(puck_init_height, device=device, requires_grad=True)
