...
class Env(Environment):
    ...
    bnds_locs = [None, [(0.2, 1.0), (math.pi/2, 3*math.pi/2)]]
    ...
    def setup_tool(self, gym, sim, env, config, tool_type):
        ...
        for i, bnds in enumerate(self.bnds_locs):
            if bnds != None:
                if type(bnds) != list:
                    ...
                    self.bnds.extend(bnds)
                    self.action_names.append(f"Radial Distance of Location {i + 1}")
                    self.action_names.append(f"Polar Direction of Location {i + 1}")
        ...
        if tool_class2 == 'wedge':
            ...
            for k in range(len(states['pose']['p'])):
                states['pose']['p'][k][0] = self.bnds_locs[1][0][0] * math.cos(self.bnds_locs[1][1][0])
                states['pose']['p'][k][1] = self.bnds_locs[1][0][0] * math.sin(self.bnds_locs[1][1][0])
            ...
            gym.set_actor_rigid_body_states(env, tool_instance2, states, gymapi.STATE_ALL)
        if tool_class1 == 'wedge':
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[4].pb_id, [self.locs[0][0], self.locs[0][1], 0.31], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            ...
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[6].pb_id, [self.locs[0][0], self.locs[0][1], 0.21], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            self.pinnSim.objects[6].friction = self.tool_instances[2][self.tool_type[2]]
        if tool_class2 == 'wedge':
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[5].pb_id, [self.bnds_locs[1][0][0], 0.0, 0.0], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            self.pinnSim.objects[5].restitution = self.tool_instances[0][self.tool_type[3]]
        elif tool_class2 == 'pendulum':
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[3].pb_id, [self.bnds_locs[1][0][0], 0.0, -0.01], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            ...
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[7].pb_id, [self.bnds_locs[1][0][0], 0.0, -0.1], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
    ...
    def generate_random_state(self, gym, sim, env, viewer, args, config):
        ...
        goal_angle = np.random.uniform(-math.pi, 0)
    ...
    def execute_action(self, gym, sim, env, viewer, args, config, action, return_ball_pos=False):
        ...
        if self.tool_type[1] != 3:
            ...
            for i in range(len(tool2_states['pose']['p'])):
                    tool2_states['pose']['p'][i][0] = action[curr_action] * math.cos(action[curr_action + 1])
                    tool2_states['pose']['p'][i][1] = action[curr_action] * math.sin(action[curr_action + 1])
        ...
        while gym.get_sim_time(sim) <= curr_time + 4.0:
            ...
            if args.simulate:
                ...
                if gym.query_viewer_has_closed(viewer):
                    break
            puck_state = gym.get_actor_rigid_body_states(env, ball_handle, gymapi.STATE_ALL)
    ...
    def execute_action_pinn(self, config, goal_pos, action, time_resolution=0.01, render=False):
        ...
        if self.tool_type[0] == 0:
            ...
            curr_action += 1
        if self.tool_type[1] == 0:
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[5].pb_id, [action[curr_action + 1] * math.cos(action[curr_action + 2]), action[curr_action + 1] * math.sin(action[curr_action + 2]), 0.0], pb.getQuaternionFromEuler([0.0, 0.0, action[curr_action]]), physicsClientId=self.pinnSim.pb_cid)
            ...
            self.pinnSim.objects[3].joint_omegas['pendulum_joint'] = 0.0
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[3].pb_id, [action[curr_action + 2] * math.cos(action[curr_action + 3]), action[curr_action + 2] * math.sin(action[curr_action + 3]), -0.01], pb.getQuaternionFromEuler([0.0, 0.0, 0.0]), physicsClientId=self.pinnSim.pb_cid)
            ...
            pb.resetBasePositionAndOrientation(self.pinnSim.objects[7].pb_id, [action[curr_action + 1] * math.cos(action[curr_action + 2]), action[curr_action + 1] * math.sin(action[curr_action + 2]), -0.1], pb.getQuaternionFromEuler([0.0, 0.0, action[curr_action]]), physicsClientId=self.pinnSim.pb_cid)
