...
class ResNet18FilmAction(nn.Module):
    ...
    def preprocess(self, observations):
        ...
        image = observations.to(self.device)
    ...
    def _forward(self, actions, features):
        ...
        # print(self.reason(features).squeeze(-1))
        # print(self.reason(features).squeeze(-1).shape)
        # input()
        # return features
    ...
    def _image_colors_to_onehot(self, indices):
        onehot = torch.nn.functional.embedding(
            indices.to(dtype=torch.long, device=self.device),
            self.embed_weights)
