...
with open('experiments/position_' + args.env + '.txt', 'a') as pos_f:
    if args.adaptive:
        pos_f.write(f'MBPO_With_GP-UCB\n')
    else:
        pos_f.write(f'MBPO_Without_GP-UCB\n')
...
class RBFPolicyNetwork(nn.Module):
    def __init__(self, input_size, output_size, num_basis_functions, bounds):
        ...
        self.gaussian_process = GaussianProcessRegressor(kernel=RBF(length_scale=1.0), n_restarts_optimizer=9)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_basis_functions):
            ...
            # rbf_values.append(torch.tensor(outputs, device=device))
    ...
    def update_GP(self, actions, difference):
        self.gaussian_process.fit(actions, difference)
...
# D = NUM_CONTEXTS  # Num rollouts for GP adaptation
...
for i in range(T):
    ...
    # pinn_state = task_env.get_final_state_via_PINN(config, action)
    reward = task_env.execute_action(gym, sim, env, viewer, args, config, action)
    # reward, fin = task_env.execute_action(gym, sim, env, viewer, args, config, action, return_ball_pos=True)
    # print(state)
    # print(action)
    # print(pinn_state)
    # print(fin)
