...
for iter in range(NUM_CONTEXTS):
    ...
    for attempt in range(NUM_ACTIONS):
        ...
        for _ in range(1000):
            ...
            reward = evalGP(state, action, actions, rewards)
            if reward > max_reward:
                max_reward = reward
                best_action = action

        def eval_curr(action):
            return -evalGP(state, action, actions, rewards)

        res = minimize(eval_curr, best_action, method='L-BFGS-B', bounds=Env.bnds)
        best_action = res.x

        reward_ideal = task_env.execute_action(gym, sim, env, viewer, args, config, best_action)
