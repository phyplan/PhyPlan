...
def make_data_noisy(data_tensor, noise, device):
    data = data_tensor.cpu().numpy()
    noisy_data = np.zeros_like(data)
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            noisy_data[i,j] = (data[i,j] * random.uniform(1.0 - noise/100, 1.0 + noise/100))
    return torch.from_numpy(noisy_data).float().to(device)
...
class PINN(nn.Module):
    ...
    def loss_CP(self, VT_f_train):
        ...
        f_x = (x_tt + 0.2 * 9.8 * torch.ones(x_tt.shape, device=self.device))
        f_v = (v_t + 0.2 * 9.8 * torch.ones(v_t.shape, device=self.device))
...
def pidnn_driver(config):
    ...
    for noise in config['NOISE']:
        with open(config['SAVE_DIR'], 'a') as file:
            file.write(str(noise) + '\n')
        for seeds in config['SEEDS']:
            print(f'++++++++++ SEED:{seeds}, NOISE: {noise} ++++++++++')
            torch.manual_seed(seeds)
            np.random.seed(seeds)
            model = PINN((0,0), make_data_noisy(VT_u_train, noise, device), make_data_noisy(X_u_train, noise, device), VT_f_train, layers, device, config, alpha, N_u, N_f)
            model.to(device)
            ...
            models.append(model)
            with open(config['SAVE_DIR'], 'a') as file:
                file.write(str(validation_loss.cpu().item())+ '\n')
