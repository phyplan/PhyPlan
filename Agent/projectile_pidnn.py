...
class PINN(nn.Module):
    ...
    def loss_CP(self, VT_f_train):
        ...
        grads = torch.ones(x.shape, device=self.device)
...
def pidnn_driver(config):
    ...
    torch.save(model, 'model_projectile.pt')
    torch.save(model.state_dict(),'weights_dict_projectile.pth')
