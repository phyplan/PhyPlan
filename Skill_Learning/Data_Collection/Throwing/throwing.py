...
for ep in range(start_ep,end_ep):
    ...
    #initial_position
    init_length_x = np.random.uniform(-0.75,0.75) #0 for checking
    ...
    while True:
        ...
        ball_states = gym.get_actor_rigid_body_states(env, ball_handle, gymapi.STATE_ALL)
        # if ball_states["pose"]["p"][0][2] < 0.1 :
        # if (t - start_time > 1.5) or (ball_states["pose"]["p"][0][2] < 0.1):
        #     break
        # if (t - start_time > 1.65):
        #     break
        if (t - start_time > 0.61) or (ball_states["pose"]["p"][0][2] < 0.1):
            break
        data_length += 1
        ...
        if config['PERCEPTION']:
            rgb_filename = "%s/rgb_%d_%f_%f_%f_%f_%f_%f_%f_%f_%f_%d.png" % (img_dir, ep, t - start_time, init_vel_z, init_vel_x, init_length_z, init_length_x, ball_states["pose"]["p"][0][2], ball_states['vel']['linear'][0][2], cur_x, ball_states['vel']['linear'][0][0], pt)
            #so that ball remains in camera frame    
            # if cur_height <= 7.85 and cur_height >= 6.15 and cur_x >= -0.85 and cur_x <= 0.85 and pt > 3:
            if cur_height <= 7.85 and cur_height >= 6.15 and cur_x >= -0.85 and cur_x <= 0.85 and pt > 0:
            ...
            ans = str(i)+ ','+str(t - start_time)+','+str(init_vel_z)+','+str(init_vel_x)+','+str(init_length_z)+','+str(init_length_x)+','+str(ball_states["pose"]["p"][0][2])+','+str(ball_states['vel']['linear'][0][2])+','+str(ball_states["pose"]["p"][0][0])+','+str(ball_states['vel']['linear'][0][0])
            fw.write(ans+ "\n")
            fw.close() 
